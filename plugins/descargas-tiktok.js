...
//m.react('🕒') 
  if (!text) return conn.reply(m.chat, `*☁️ Iɴɢʀᴇsᴇ Uɴ Eɴʟᴀᴄᴇ Dᴇ Vɪᴅᴇᴏ Dᴇ TɪᴋTᴏᴋ.*\n\n*💌 Eᴊᴇᴍᴘʟᴏ:* _${usedPrefix + command} https://vm.tiktok.com/ZM6UHJYtE/_`, m, rcanal);
  if (!/(?:https:?\/{2})?(?:w{3}|vm|vt|t)?\.?tiktok.com\/([^\s&]+)/gi.test(text)) throw `*☁️ Iɴɢʀᴇsᴇ Uɴ Eɴʟᴀᴄᴇ Dᴇ Vɪᴅᴇᴏ Dᴇ Tɪᴋᴛᴏᴋ.*\n\n*💌 Eᴊᴇᴍᴘʟᴏ:* _${usedPrefix + command} https://vm.tiktok.com/ZM6UHJYtE /_`;
  const texto = `_💌 @${m.sender.split`@`[0]}  ᩭ✎Enviando Video, espere un momento...._`;
  // let buttons = [{ buttonText: { displayText: '♫ 𝙰𝚄𝙳𝙸𝙾 ♫' }, buttonId: `${usedPrefix}tomp3` }]
  try {
m.react('🕒') 
    const aa = {quoted: m, userJid: conn.user.jid};
    const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: texto, contextInfo: {externalAdReply: {title: packname, body: wm, sourceUrl: yt}, mentionedJid: [m.sender]}}}, aa);
    await conn.relayMessage(m.chat, prep.message, {messageId: prep.key.id, mentions: [m.sender]});
    const dataFn = await conn.getFile(`${CFROSAPI}/api/tiktokv2?url=${args[0]}`);
    const desc1n = `_💌  ᩭ✎Tiktok sin marca de agua descargado con éxito by ✰ 𝙺𝚊𝚗𝙱𝚘𝚝 ✰_`;
  ...
  try {
    ...
    const desc1 = `_💌  ᩭ✎Tiktok sin marca de agua descargado con éxito by ✰ 𝙺𝚊𝚗𝙱𝚘𝚝 ✰_`;
    await conn.sendMessage(m.chat, {video: {url: dataF.play}, caption: desc1}, {quoted: fkontak});
  } catch (e1) {
    try {
      const tTiktok = await tiktokdlF(args[0]);
      // let desc2 = `🔗 *Url:* ${tTiktok.video}`
      const desc2 = `_💌  ᩭ✎Tiktok sin marca de agua descargado con éxito by ✰ 𝙺𝚊𝚗𝙱𝚘𝚝 ✰_`;
      ...
      try {
        ...
        const te = `_💌  ᩭ✎Tiktok sin marca de agua descargado con éxito by ✰ 𝙺𝚊𝚗𝙱𝚘𝚝 ✰_`;
        ...
        try {
          ...
          const cap = `_💌  ᩭ✎Tiktok sin marca de agua descargado con éxito by ✰ 𝙺𝚊𝚗𝙱𝚘𝚝 ✰_`;
